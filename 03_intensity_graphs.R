...
G3Bp1_intensities_plot <- left_join(mean_G3Bp1, metadata) |> slice(-3)|> slice(-5) |> ggplot(aes(x = factor(Name), y = Mean_G3Bp1_Intensity, fill = G3Bp1)) + geom_bar(stat = "identity") + labs(x = "Image Number", y = "Mean G3Bp1 Intensity") + geom_errorbar(aes(ymin = Mean_G3Bp1_Intensity - SD_G3Bp1_Intensity, ymax = Mean_G3Bp1_Intensity + SD_G3Bp1_Intensity), width = 0.2)  + ggtitle("Mean G3Bp1 Intensity per Cell") + theme_bw() + scale_fill_manual(values = c("pos" = "forestgreen", "neg" = "gray")) + theme(axis.text.x = element_text(angle = 30, hjust = 1))
...
HuR_intensities_plot <- left_join(mean_HuR, metadata)|> slice(-3)|> slice(-5)  |> ggplot(aes(x = factor(Name), y = Mean_HuR_Intensity, fill = HUR)) + geom_bar(stat = "identity") + labs(x = "Image Number", y = "Mean HuR Intensity") + geom_errorbar(aes(ymin = Mean_HuR_Intensity - SD_HuR_Intensity, ymax = Mean_HuR_Intensity + SD_HuR_Intensity), width = 0.2)  + ggtitle("Mean HuR Intensity per Cell") + theme_bw() + scale_fill_manual(values = c("pos" = "red3", "neg" = "gray")) + theme(axis.text.x = element_text(angle = 30, hjust = 1))
...
TIAR_intensities_plot <- left_join(mean_TIAR, metadata) |> slice(-3) |> slice(-5) |> ggplot(aes(x = factor(Name), y = Mean_TIAR_Intensity, fill = TIAR)) + geom_bar(stat = "identity") + labs(x = "Image Number", y = "Mean TIAR Intensity") + geom_errorbar(aes(ymin = Mean_TIAR_Intensity - SD_TIAR_Intensity, ymax = Mean_TIAR_Intensity + SD_TIAR_Intensity), width = 0.2)  + ggtitle("Mean TIAR Intensity per Cell") + theme_bw() + scale_fill_manual(values = c("pos" = "gold1", "neg" = "gray")) + theme(axis.text.x = element_text(angle = 30, hjust = 1))
