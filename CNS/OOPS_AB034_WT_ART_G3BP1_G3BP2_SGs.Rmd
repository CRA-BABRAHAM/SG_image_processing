...
directory <- "C:/Users/anaso/OneDrive - BABRAHAM/experiments/OOPS_AB034_CN/tif for cellprofiler"
...
var_names <- ls(pattern = "OOPS_AB034_")
for (old_name in var_names) {
  # Generate the new name by removing "prefix"
  new_name <- gsub("OOPS_AB034_", "", old_name)
...
Metadata <- Metadata |>
  mutate(Stressor = case_when(
    str_detect(FileName_G3Bp1, "imat") ~ "Imatinib treated",
...
Metadata <- Metadata |> 
  mutate(cell_line = case_when(
    ...
    str_detect(FileName_G3Bp1, "ART") ~ "ART KO",
    TRUE ~ "ERROR"
  ))
...
Image_filtered <- Image[, c("ImageNumber", "Count_SG_G3Bp1_in_cells" , "Count_SG_G3Bp2_in_cells" , "Count_G3Bp1_G3Bp2_SG_in_cells" , "Count_cell")]
...
Image_filtered <- Image_filtered |> 
  mutate(SG_per_cell_G1G2 = Count_G3Bp1_G3Bp2_SG_in_cells/Count_cell)
```
...
Image_filtered_summary$Stressor <- factor(Image_filtered_summary$Stressor, 
                                  levels = c("Untreated", "Imatinib treated"))
# set colours
manual_colors <- c("Imatinib treated" = "#00bfc4", "Untreated" = "#F8766D")  

#Number of SGs per cell plot - G3BP1
SG_mean_G3Bp1 <- Image_filtered_summary |>
  ggplot(aes(x = cell_line, y = mean_SG_per_cell_G1, fill = Stressor)) + 
  ...
  labs(
    title = "Average number of G3Bp1-positive SGs",
...
SG_mean_G3Bp2 <- Image_filtered_summary |>
  ggplot(aes(x = cell_line, y = mean_Ratio_G3Bp2, fill = Stressor)) + 
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +  
  geom_errorbar(
  ...
  labs(
    title = "Average number of G3Bp2-positive SGs",
    y = "Average number of SGs per cell",
  ) + theme_minimal() + 
  scale_fill_manual(values = manual_colors) + scale_y_continuous(limits = c(0, 1.8)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", legend.title = element_blank(), axis.title.x = element_blank()) + theme(
...
SG_mean_G1_G2 <- Image_filtered_summary |>
  ggplot(aes(x = cell_line, y = mean_Ratio_G3Bp1_G3Bp2, fill = Stressor)) + 
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +  
  geom_errorbar(
  ...
  labs(
    title = "Average number of SGs per cell (based on G3Bp1/G3Bp2 signal overlap)",
    y = "Average number of SGs per cell",
  ) + theme_minimal() + 
  scale_fill_manual(values = manual_colors) + scale_y_continuous(limits = c(0, 1.8)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", legend.title = element_blank(), axis.title.x = element_blank()) + theme(
...
new_df <- data.frame(ImageNumber = 1:53,
                     Cell_count = 0,
...
cells_with_SG <- cells_with_SG |> 
  mutate(Ratio_G3Bp1_G3Bp2 = Children_G3Bp1_G3Bp2_SG_Count/Cell_count * 100)
```
...
percentage_cells_with_SG_G3Bp1 <- cells_with_SG_summary |>
  ggplot(aes(x = cell_line, y = mean_Ratio_G3Bp1, fill = Stressor)) + 
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +  
  geom_errorbar(
  ...
  labs(
    title = "Percentage of cells with G3Bp1-positive SGs",
    y = "% cells with SGs",
  ) + theme_minimal() + 
   scale_fill_manual(values = manual_colors) + scale_y_continuous(limits = c(0, 6)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", legend.title = element_blank(), axis.title.x = element_blank()) + theme(
         panel.background = element_rect(fill='transparent'),
         plot.background = element_rect(fill='transparent', color=NA),
         panel.grid.major = element_blank(),
...
#PEercentage plot for G3BP2
percentage_cells_with_SG_G3Bp2 <- cells_with_SG_summary |>
  ggplot(aes(x = cell_line, y = mean_Ratio_G3Bp2, fill = Stressor)) + 
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +  
  geom_errorbar(
    aes(ymin = mean_Ratio_G3Bp2 - se_Ratio_G3Bp2, ymax = mean_Ratio_G3Bp2 + se_Ratio_G3Bp2),
  ...
  labs(
    title = "Percentage of cells with G3Bp2-positive SGs",
    y = "% cells with SGs",
  ) + theme_minimal() + 
...
percentage_cells_with_SG_G1_G2 <- cells_with_SG_summary |>
  ggplot(aes(x = cell_line, y = mean_Ratio_G3Bp1_G3Bp2, fill = Stressor)) + 
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +  
  geom_errorbar(
    aes(ymin = mean_Ratio_G3Bp1_G3Bp2 - se_Ratio_G3Bp1_G3Bp2, ymax = mean_Ratio_G3Bp1_G3Bp2 + se_Ratio_G3Bp1_G3Bp2),
  ...
  labs(
    title = "Percentage of cells with G3Bp1/G3Bp2-positive SGs",
    y = "% cells with SGs",
  ) + theme_minimal() + 
